...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True, 'target': target, 'message': Message(
            'A lighting bolt strikes the {0} with a loud thunder for {1} points of damage.'.format(target.name, damage))})
...
def cast_fireball(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False, 'message': Message('You can\'t target a tile outside of your field of view.', libtcod.yellow)})
        return results
    ...
    for entity in entities:
        if entity.distance(target_x, target_y) <= radius and entity.fighter:
            results.append({'message': Message('The {0} gets burned for {1} points of damage.'.format(entity.name, damage), libtcod.orange)})
...
def cast_confuse(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False, 'message': Message(
            'You can\'t target a tile outside of your field of view.', libtcod.yellow)})
        return results
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({'consumed': True, 'message': Message(
                'The eyes of the {0} look vacant as he starts to stumble around!'.format(entity.name), libtcod.light_green)})
